...
class PyfficeUnit(object):
    ...
    def __init__(self, cfg=None):
        ...
        # Initialize config as Instruct for proper override chain
        try:
            if condor:
                self.config = condor.Instruct(cfg) if isinstance(cfg, dict) else condor.Instruct({})
            else:
                self.config = cfg if isinstance(cfg, dict) else {}
        except:
            self.config = cfg if isinstance(cfg, dict) else {}
        # Initialize unit as Instruct for proper override chain
        try:
            if condor:
                self.unit = condor.Instruct({})
            else:
                self.unit = {}
        except:
            self.unit = {}
