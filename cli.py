...
@click.option('--agent', '-a', default=None, help='Agent name for multi-agent coordination')
@click.pass_context
def cli(ctx: click.Context, verbose: bool, quiet: bool, config: Optional[str], agent: Optional[str]) -> None:
    """Pyffice - Comprehensive document and media framework."""
    ctx.ensure_object(dict)
    ctx.obj['verbose'] = verbose
    ctx.obj['quiet'] = quiet
    ctx.obj['config'] = config
    ctx.obj['agent'] = agent  # Agent awareness for multi-agent coordination
